...
const MyProfile = () => {
  ...
  const getMyProfileDetails = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMyProfileData(formattedData)
    ...
    return (
      <div className="user-profile-success-view-container">
        ...
        <div className="user-profile-posts-section">
          ...
          {posts.length === 0 ? renderNoPostsView() : renderPostsView()}
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      <Header />
      <div className="user-profile-container">{renderUserProfileView()}</div>
    </>
  )
}
